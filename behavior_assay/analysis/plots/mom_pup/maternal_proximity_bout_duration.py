...
METRIC = "avg_body_scale_proximity_bout_duration_sec"

DEFAULT_STEM = "maternal_proximity_mean_bout_duration_1s"
...
def plot_duration(
    ...
    ax.set_ylabel("Mean bout duration (s)", fontsize=20, labelpad=12)
    duration_label = (
        "all bouts"
        if min_duration_sec <= 0
        else f"at least {min_duration_sec:g} s"
    )
    ax.set_title(
        f"Maternal proximity bout duration ({duration_label})",
        fontsize=26,
        pad=16,
    )
