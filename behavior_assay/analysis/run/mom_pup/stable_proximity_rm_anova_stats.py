...
SOURCE_METRIC = "total_stable_proximity_time_sec"

def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Run a 2 x 3 mixed repeated-measures ANOVA for total stable "
            "maternal proximity time and Holm-adjusted within-PND group "
            "contrasts."
        )
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output-dir",
        default=str(ROOT / "output" / "mom_pup" / "stable_proximity"),
    )
    ...
    print("Stable proximity mixed repeated-measures ANOVA")
