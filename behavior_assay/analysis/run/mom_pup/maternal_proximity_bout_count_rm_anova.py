...
SOURCE_METRIC = "n_body_scale_proximity_bouts"

def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Run a 2 x 3 mixed repeated-measures ANOVA for the number of "
            "body-scale maternal proximity bouts lasting at least one second."
        )
    )
    parser.add_argument(
        "--metrics",
    ...
    parser.add_argument(
        "--output-dir",
        default=str(
            ROOT / "output" / "mom_pup" / "body_scale_proximity_bouts_1s"
        ),
    )
    ...
    print("Maternal proximity bout-count mixed repeated-measures ANOVA")
